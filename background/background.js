        ...
        port.onMessage.addListener(async (event) => {
            ...
            switch (event.type) {
                ...
                case 'GENERATE_OVERVIEW':
                    ...
                    const {overviewWords } = await generateOverview(event.props.content);
                    if (app.states.error) app.contentPort.postMessage({ type: 'ERROR', props: { error: app.error } });
                    if (app.states.overview) app.contentPort.postMessage({ type: 'OVERVIEW', props: {words: overviewWords } });
                    break;
